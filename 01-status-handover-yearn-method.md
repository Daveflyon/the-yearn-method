...
Last updated 8 Sep 2026, after floating expand/collapse-all, Lucy Bronze (anxiety/dyslexia/ADHD/autism), and inline quote-sourcing went live.
...
- All sections now default to collapsed on page load (previously "What This Is" and "Pick Today's Story" opened by default). The Expand/Collapse-all toggle is now a floating button, fixed bottom-left of the viewport (mirroring the existing "Jump to" button fixed bottom-right) so it's reachable at any scroll position, not just at the top of the page — label shortened to "Expand all" / "Collapse all" and both buttons sized down so they sit in the corners without covering page content. Added 90px of bottom padding to the page so the last section always clears the floating buttons.
